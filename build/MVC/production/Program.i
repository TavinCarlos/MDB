...
mdb_poll_result_t MDB_ParsePoll(uint8_t *buf, uint8_t len);


void MDB_Core_Task(void);
uint8_t MDB_Core_HasFrame(void);

uint8_t MDB_ParsePoll_Status(uint8_t *buf, uint8_t len);
...
typedef enum
  ...
  MDB_EXP_ID_WAIT,
  MDB_EXP_FEATURE,
  MDB_EXP_FEATURE_WAIT,
  MDB_TUBE_STATUS,
  MDB_TUBE_STATUS_WAIT,
  MDB_ENABLE_COINS,
  MDB_ENABLE_COINS_WAIT,
  MDB_DISBLE_COINS,
  MDB_DISABLE_COINS_WAIT,
  MDB_POLL,
  MDB_POLL_WAIT,
  MDB_ERROR,
  MDB_WAIT
...
typedef enum
{
    ...
    MDB_STATE_VEND,
    MDB_STATE_ERROR
} fsm_mdb_link_state_t;


void FSM_MDB_Link_Init(void);
void FSM_MDB_Link_Task(void);
void FSM_MDB_Link_Wait(void);
void FSM_MDB_Link_HandleFrame(mdb_frame_t *f);
# 13 "Include/mdb.h" 2
# 138 "Include/mdb.h"
typedef enum
{
...
typedef struct {
    uint8_t Mode;
    uint16_t SaldoDepositado;
    uint8_t ProductoSeleccionado;
    uint8_t CoinRouting;
} VendingMachine_t;
...
extern coin_changer_info_t CoinChangerInfo;
...
void MDB_Send_EnableAllCoins();
void MDB_Send_Command(uint8_t command, uint8_t subCommand);

void Dispense1(uint8_t x);

void USART_Write_Msg(uint8_t msg);
void USART_Write_Cmd(uint8_t *datos, uint8_t longitudDatos);


void SendDiagnosticStatus();
void MDB_Send_DisableAllCoins();
void Dispense1(uint8_t x);

void MDB_StateMachines2();

void Mdb_Init();

extern uint16_t SaldoDepositado;
...
extern struct DATOS_VENTA DatosVenta;
extern volatile uint8_t Rx_data[80];
extern volatile uint8_t received_data;

extern volatile uint8_t rx_frame_hint;

extern VendingMachine_t myMachine;
# 9 "Program.c" 2
# 1 "Include/botones.h" 1
# 12 "Include/botones.h"
extern volatile _Bool BtnA0_Event;
extern volatile _Bool BtnA1_Event;
extern volatile _Bool BtnA2_Event;

extern volatile _Bool BtnD0_Event;
extern volatile _Bool BtnD1_Event;
extern volatile _Bool BtnD2_Event;

extern volatile _Bool BtnD0_LongEvent;


void Buttons_Init(void);
void Buttons_Task(void);
# 10 "Program.c" 2
# 1 "Include/hal_uart.h" 1
...
void USART_Write_Msg(uint8_t msg);
void hal_UART_Write_Cmd(uint8_t *datos, uint8_t longitudDatos);
# 11 "Program.c" 2
# 1 "Include/eeprom.h" 1
# 23 "Include/eeprom.h"
uint8_t EEPROM_Read(uint8_t address);

void FactoryReset_Parameters(uint8_t precio1, uint8_t precio2, uint8_t tiempo1, uint8_t tiempo2);


_Bool Eeprom_Check_Signature(uint8_t address, uint8_t value) ;

void FactoryReset_Contador_uint16(uint8_t address, uint16_t value);


char Eeprom_Read_uint16(uint8_t address, uint16_t *valid);
void Eeprom_Write_uint16(uint8_t address, uint16_t value);
...
typedef enum {
    GLOBAL_EVT_NONE,
    GLOBAL_EVT_INIT_DONE,
    GLOBAL_EVT_CREDIT_UPDATED,
    GLOBAL_EVT_NO_CREDIT,
    GLOBAL_EVT_SELECTION,
    GLOBAL_EVT_CANCEL,
    GLOBAL_EVT_MDB_ERROR
} global_event_t;
...
typedef enum
{
    ...
    VENTA_EVT_COIN_ACCEPTED,
    VENTA_EVT_COIN_REJECTED,


    VENTA_EVT_RETURN_REQUEST,


    VENTA_EVT_CHANGER_BUSY,
    VENTA_EVT_CHANGER_READY,
    VENTA_EVT_NO_CREDIT,


    VENTA_EVT_CHANGER_ERROR,
    VENTA_EVT_ROM_ERROR,
    VENTA_EVT_TUBE_JAM,
    VENTA_EVT_DOUBLE_ARRIVAL,
    VENTA_EVT_ROUTE_ERROR,
    VENTA_EVT_COIN_JAM,
    VENTA_EVT_REJECTER_ERROR,


    VENTA_EVT_JUST_RESET,
    VENTA_EVT_ACCEPTOR_UNPLUGGED,


    VENTA_EVT_ENABLE_COINS,
    VENTA_EVT_DISABLE_COINS

} venta_event_t;
# 13 "Program.c" 2
# 1 "Include/fsm_global.h" 1
# 19 "Include/fsm_global.h"
typedef enum {
    GLOBAL_INIT,
    GLOBAL_IDLE,
    GLOBAL_CREDIT,
    GLOBAL_VENDING,
    GLOBAL_OUT_OF_SERVICE
} global_state_t;


void GlobalFSM_Init(void);
void GlobalFSM_Dispatch(global_event_t evt);
void GlobalFSM_Task(void);


global_state_t GlobalFSM_GetState(void);
# 14 "Program.c" 2
# 1 "Include/fsm_coin_logic.h" 1
# 17 "Include/fsm_coin_logic.h"
typedef enum {
    COIN_IDLE,
    COIN_CREDIT,
    COIN_DISABLED,
...
void FSM_Coin_Logic(void);


void CoinFSM_Dispatch(mdb_event_t evt, uint8_t *frame, uint8_t len);
# 15 "Program.c" 2
# 1 "Include/fsm_diag.h" 1
# 15 "Include/fsm_diag.h"
void FSM_Diag(void);
# 16 "Program.c" 2
# 1 "Include/venta_fsm.h" 1
# 17 "Include/venta_fsm.h"
typedef enum
{
  VENTA_IDLE,
  VENTA_WAIT_CREDIT,
...
void VentaFSM_Event(venta_event_t evt, uint16_t param);
int8_t VendFSM_IsDone(void);
void Venta_FSM(void);
# 17 "Program.c" 2
# 1 "Include/ui_fsm.h" 1
# 11 "Include/ui_fsm.h"
void UI_Menu_Task(void);
void UI_Menu_Task(void);
void LCD_ShowNormal(void);
void Display(unsigned short *sec, unsigned short *min, unsigned short *hour, unsigned short *wd, unsigned short *day, unsigned short *month, unsigned short *year);
void DisplaySaldo();
void LCD_Task(void);
void UI_Task(void);
void Service_Task(void);
void Service_Home(void);
void Service_Status(void);
void Service_Errors(void);
void Service_TestCoin(void);
void Service_Tubes(void);
void Service_ResetErrors(void);
void ServiceMenuUp(void);
void ServiceMenuDown();
void LCD_ShowService(void);
void UI_Menu_Task(void);
void Menu_Task(void);

extern unsigned char flag_jam;
extern unsigned char flag_tube_full;
extern unsigned char flag_sensor_error;
extern unsigned char flag_inhibited;

void UI_Running_Task(void);

void UI_Services_ShowService(void);

void MDB_PublishDiag(uint8_t *dataX, uint8_t len);
void LCDWriteFromROM(char row, char col, const char *text);
void Lcd_PrintFixed(char row, char col, char *text, uint8_t width);
void LCD_PrintValue(char row, char col, uint16_t value, char width);


void UI_FSM_Init(void);

void UI_SetIdle(void);
void UI_SetCredit(uint16_t credit);
void UI_SetSelect(void);
void UI_SetDispensing(void);
void UI_SetPrice(uint16_t price);
void UI_SetVend(void);
void UI_SetFinished(void);
void UI_SetOutOfService(void);



void App_Task(void);
# 18 "Program.c" 2
# 1 "Include/uart_compat.h" 1
...
uint8_t MDB_UART_BytesAvailable(void);
void MDB_UART_Flush(void);
uint8_t MDB_UART_Read(uint8_t *byte, uint8_t *bit9);
# 21 "Program.c" 2
...
# 23 "Program.c" 2
# 32 "Program.c"
struct DATOS_VENTA DatosVenta = {0, 0, 0, 0, 0, 0};

volatile uint8_t Rx_data[80];
volatile uint8_t RX_mode[80];
...
VendingMachine_t myMachine = {MDB_RESET, MDB_WAIT, 0, 0};
...
void main(void) {
    ...
    FactoryReset_Parameters(8, 18, 10, 11);
    DatosVenta.Precio1 = (uint16_t) EEPROM_Read(0x01) * 100;
    DatosVenta.Precio2 = (uint16_t) EEPROM_Read(0x02) * 100;
    DatosVenta.Tiempo1 = (uint16_t) EEPROM_Read(0x03);
    DatosVenta.Tiempo2 = (uint16_t) EEPROM_Read(0x04);

    DatosVenta.Contador1 = 0;
    DatosVenta.Contador2 = 0;

    FactoryReset_Contador_uint16(0x05, 0);
    FactoryReset_Contador_uint16(0x09, 0);

    Eeprom_Read_uint16(0x05, &DatosVenta.Contador1);
    Eeprom_Read_uint16(0x09, &DatosVenta.Contador2);

    LCD_HAL_Init();
    LCD_FSM_Init();


    LCD_Write_Buffer(1, 1, "SISTEMA INICIADO");
    LCD_Write_Buffer(2, 4, "MDB @ 9600");
    LCD_Write_Buffer(4, 1, "PURIFICADORA");
    t_delay_ms(50);
    ...
    GlobalFSM_Dispatch(GLOBAL_EVT_INIT_DONE);
