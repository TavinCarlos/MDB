...
# 1 "Include/mdb_link.h" 1
# 13 "Include/mdb_link.h"
    typedef enum {
        ST_MDB_RESET = 0,
        ST_MDB_RESET_WAIT,
        ST_MDB_JUST_RESET,
...
void MDB_Link_Task_Command(void)
{
    switch (current_state) {
        ...
        case ST_MDB_JUST_RESET:
            send_command(0x0B, 0);
            TimerNB_Start(&NB_JustResetTimeout, 50);
            TransitionTo(ST_MDB_JUST_RESET_WAIT, 50);
            break;
        ...
        case ST_MDB_EXP_DIAG_STATUS:
            if (coin_changer_info.CoinChangerFeatureLevel >= 3) {
                send_command(0x0F, 0x05);
                TimerNB_Start(&NB_EXP_DIAG_TimeOut, 80);
                TransitionTo(ST_MDB_EXP_DIAG_STATUS_WAIT, 80);
            }
            else
                TransitionTo(ST_MDB_TUBE_STATUS, 200);
            break;
        ...
        case ST_MDB_EXP_DIAGNOSTIC:
            if (coin_changer_info.CoinChangerFeatureLevel >= 3) {
                send_command(0x0F, 0x05);
                TimerNB_Start(&NB_EXP_DIAG_TimeOut, 80);
                TransitionTo(ST_MDB_EXP_DIAGNOSTIC_WAIT, 80);
            }
            else
