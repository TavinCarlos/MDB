...
_Bool BtnD1_IsPressed(void);
_Bool BtnD2_IsPressed(void);
# 7 "sources/UI_Config.c" 2
# 1 "Include/ui_config.h" 1
...
static _Bool cfg_edit;
static _Bool cfg_blink;

static uint32_t cfg_blink_timer;

static uint16_t cfg_edited[(sizeof(ui_params) / sizeof(ui_params[0]))];
...
static void LoadConfig(void) {
    for (uint8_t i = 0; i < (sizeof(ui_params) / sizeof(ui_params[0])); i++) {
...
static void display_config_values(void) {
    ...
    for (char i = start; i < start + 4 && i < ((sizeof(ui_params) / sizeof(ui_params[0])) + 2); i++) {

        LCD_Write_Buffer(row, 1, (cfg_menu == i) ? ">" : " ");

        if (i < (sizeof(ui_params) / sizeof(ui_params[0]))) {

            strncpy(buff, ui_params[i].label, strlen(ui_params[i].label) - 1);
            buff[strlen(ui_params[i].label) - 1] = '\0';

            LCD_Write_Buffer(row, 2, buff);

            if (cfg_edit && cfg_menu == i)
                LCD_Write_Buffer(row, 13, cfg_blink ? " " : "*");
            else
                LCD_Write_Buffer(row, 13, " ");

            UInt16ToStr(cfg_edited[i], buff);
            Trim(buff);
            PadLeft(buff, 4, ' ');
            LCD_Write_Buffer(row, 15, buff);

        }
        else if (i == (sizeof(ui_params) / sizeof(ui_params[0]))) {

            LCD_Write_Buffer(row, 2, "Guardar");

        }
        else {

            LCD_Write_Buffer(row, 2, "Cancelar");
        }
...
static _Bool ButtonRepeat(_Bool pressed, btn_repeat_t *r) {
    uint32_t now = Millis();
    ...
    if (now - r->hold_start < 500)
        return 0;

    uint32_t period =
            (now - r->hold_start > 2000) ? 80 : 200;
...
void UI_Config_Contrl_Task(void) {
    ...
    if (BtnD0_Event) {
        ...
        if (cfg_menu < (sizeof(ui_params) / sizeof(ui_params[0])))
            cfg_edit = !cfg_edit;
        else if (cfg_menu == (sizeof(ui_params) / sizeof(ui_params[0]))) {
            SaveConfig();
    ...
    if (up) {
        if (cfg_edit && cfg_menu < (sizeof(ui_params) / sizeof(ui_params[0]))) {
    ...
    if (down) {
        ...
        else if (cfg_menu < ((sizeof(ui_params) / sizeof(ui_params[0])) + 2) - 1) {
            cfg_menu++;
            rep_up.active = 0;
            rep_down.active = 0;
        }
