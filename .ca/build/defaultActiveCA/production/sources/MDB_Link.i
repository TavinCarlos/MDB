...
extern volatile uint8_t Rx_data[36+10];
...
# 7 "sources/MDB_Link.c" 2
# 1 "Include/mdb_status.h" 1
# 17 "Include/mdb_status.h"
    typedef struct {
        ...
        uint8_t tube_sensor_err;
        uint8_t double_coin;
        uint8_t tube_jam;
        uint8_t routing_error;
        uint8_t coin_jam;


        uint8_t acceptor_removed;
        uint8_t rom_error;
        uint8_t fraud;
...
# 8 "sources/MDB_Link.c" 2
# 1 "Include/fsm_mdb_link.h" 1
# 13 "Include/fsm_mdb_link.h"
    typedef enum {
        ST_MDB_RESET = 0,
        ST_MDB_RESET_WAIT,
        ST_MDB_JUST_RESET,
...
# 13 "sources/MDB_Link.c" 2
# 1 "Include/checksum.h" 1
...
# 14 "sources/MDB_Link.c" 2
# 27 "sources/MDB_Link.c"
typedef struct
{
    uint16_t value;
    uint8_t HasValue;
...
static void MDBLinkSendCommand(uint8_t command, uint8_t subCommand);
...
static MDB_Link_State_t current_state = ST_MDB_RESET;
static MDB_Link_State_t next_state;
...
static void MDB_Link_Log_TX(uint8_t cmd, uint8_t sub, uint8_t len) {
...
static void MDB_Link_To_PC(const uint8_t *dta, uint8_t len) {
    if (len > 36+10)
        len = 36+10;
...
static void MDBSendACK(void) {
    hal_UART_SendACK();
}


static void MDB_SendCommand_Refund(uint16_t value) {
    if (value >= 1) {
        uint8_t monedas = (uint8_t) (value / coin_changer_info.CoinScalingFactor);
        uint8_t cmd[4];
        cmd[0] = 0x0F;
        cmd[1] = 0x02;
        cmd[2] = monedas;
...
void MDB_Link_Init(void) {
    MDBLinkSendCommand(0x08, 0);
    TransitionTo(ST_MDB_RESET, 1000);
}


static MDB_Event_t ParsePoll(const uint8_t *buf, uint8_t len) {

    if (len == 1 && buf[0] == 0x00)
        return EVT_MDB_ACK;


    if (len == 3 && ((((buf[0]) & 0xC0u) == 0x40u))) {
...
static void MDBLinkSendCommand(uint8_t command, uint8_t subCommand) {
    ...
    uint8_t lenLog = (command == 0x0C) || (command == 0x0F) ? 2 : 1;
    MDB_Link_Log_TX(command, subCommand, lenLog);
}
...
void MDB_Link_Task(void)
{
    ...
    switch (current_state) {
        ...
        case ST_MDB_RESET:
            ...
            TimerNB_Start(&NB_ResetTimeOut, 5);
            Dispatcher_Post(SRC_MDB, EVT_MDB_CHANGER_RESET, 0);
            break;



        case ST_MDB_RESET_WAIT:
            if (MDB_Core_FrameAvailable()) {
                f = MDB_Core_GetFrame();
                MDB_Link_To_PC(f->rxdata, f->len);
                if (f->len == 1 && f->rxdata[0] == 0x00) {
                    MDBSendACK();
        ...
        case ST_MDB_JUST_RESET:
            MDBLinkSendCommand(0x0B, 0);
            TimerNB_Start(&NB_JustResetTimeout, 5);
            TransitionTo(ST_MDB_JUST_RESET_WAIT, 5);
            break;

        case ST_MDB_JUST_RESET_WAIT:
            if (MDB_Core_FrameAvailable()) {
                f = MDB_Core_GetFrame();
                MDB_Status_Clear();
                MDB_Link_To_PC(f->rxdata, f->len);
        ...
        case ST_MDB_SETUP:
            ...
            MDBLinkSendCommand(0x09, 0);
            TransitionTo(ST_MDB_SETUP_WAIT, 60);
            TimerNB_Start(&NB_SetupTimeOut, 40);
# 326 "sources/MDB_Link.c"
            break;
        ...
        case ST_MDB_EXP_ID:
            if (coin_changer_info.CoinChangerFeatureLevel >= 3) {
                TransitionTo(ST_MDB_EXP_ID_WAIT, 100);
                MDBLinkSendCommand(0x0F, 0x00);
                TimerNB_Start(&NB_EXPIDTimeOut, 70);
        ...
        case ST_MDB_TUBE_STATUS:
            MDBLinkSendCommand(0x0A, 0);
            TransitionTo(ST_MDB_TUBE_STATUS_WAIT, 50);
            TimerNB_Start(&NB_TubeStatusTimeOut, 70);
            break;
        ...
        case ST_MDB_DISABLE_COINS_WAIT:
            ...
            break;
# 561 "sources/MDB_Link.c"
        case ST_MDB_POLL:
            MDBLinkSendCommand(0x0B, 0x00);
            TransitionTo(ST_MDB_POLL_WAIT, 10);
            TimerNB_Start(&NB_PollTimeOut, 10);
            break;
        ...
        case ST_MDB_POLL_WAIT:
        {
            if (MDB_Core_FrameAvailable()) {
                f = MDB_Core_GetFrame();
                MDB_Status_Clear();
                MDB_Event_t evt = ParsePoll(f->rxdata, f->len);
                ...
                switch (evt) {
                    case EVT_MDB_ACK:
                        ...
                        if (RefundValue.HasValue && RefundValue.value) {
                            MDB_SendCommand_Refund(RefundValue.value);
...
# 595 "sources/MDB_Link.c"
                    ...
                    case EVT_MDB_COIN_VALID:
                        ...
                        if (((f->rxdata[0] + f->rxdata[1]) & 0xFF) == f->rxdata[2]) {
                            ...
                            uint8_t CoinTypeDeposited = f->rxdata[0] & 0x0F;
